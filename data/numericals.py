...
def draw_point_reinforce(
  ...
  ax.annotate(
      name, naming_position(ax, p, lines, circles), color=ncolor, fontsize=15
  )
...
def draw_line(
    ax: matplotlib.axes.Axes, line: Line, color: Any = 'white', show_len: bool = False
) -> tuple[Point, Point]:
  """Draw a line."""
  ...
  if show_len:
    ...
    # 复杂样例
    if not( "AB" in line_name or "BC" in line_name or "AE" in line_name or "CE" in line_name or "BF" in line_name):
      return p1, p2, None, None
    
    # 数值原语
    # if not( "AD" in line_name):
    #   return p1, p2, None, None
    ...
    # print(line_name, line_len)
    name=line_name+" = "+"{:.2f}".format(line_len)
    ax.annotate(
      name, ((p1.x+p2.x)/2, (p1.y+p2.y)/2), color=color, fontsize=8
    )
...
def _draw(
  ...
  for l in lines:
    if show_len:
      # 显示线段长度，可存储进语料中
      p1, p2, line_len, line_name = draw_line(ax, l, color=lcolor, show_len=True)
      # print(line_len)
      if line_len is not None:
        line_sict[line_name]=line_len
    else:
...
def _draw_reinforce(
  ...
  if get_theme() == 'dark':
    # pcolor, lcolor, ccolor = 'white', 'white', 'cyan'
    colors = ['red', 'green', 'blue', 'orange', 'magenta', 'purple', 'grey', 'cyan', 'white']
  elif get_theme() == 'light':
    # pcolor, lcolor, ccolor = 'black', 'black', 'blue'
...
def draw_reinforce(
  ...
  _draw_reinforce(ax, points, lines, circles, goal, equals, highlights)
