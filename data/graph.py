...
class Graph:
  ...
  def build_problem(
    ...
    while not check:
      try:
        ...
        added = []
        plevel = 0
        for i, clause in enumerate(pr.clauses):
          ...
          #       g.type2nodes[Line],
          #       g.type2nodes[Circle],
          #       g.type2nodes[Segment],
          #       theme='',
          #       save_to=f"debug/img/process-{i}.jpg")
  ...
  def add_para(
    ...
    # bd, why1 = self.get_line_thru_pair_why(b, d)
    # ac, why2 = self.get_line_thru_pair_why(a, c)
    
    deps.algebra = ab._val, cd._val
    # deps.algebra = ab._val, cd._val, bd._val, ac._val
  ...
  def add_clause(
    ...
    def range_fn() -> (
      ...
      for c in clause.constructions:
        ...
        for n in cdef.numerics:
          ...
          # print("middle:::", n.name, args)
          # args=['A', 'B', 'C']
          # print("middle:::", n.name, args)
          to_be_intersected += nm.sketch(n.name, args)
          # print(to_be_intersected)
    ...
    # print('self.type2nodes', self.type2nodes[Point])
    ...
    for c, deps in zip(clause.constructions, new_points_dep):
      ...
      for points, bs in cdef.basics:
        ...
        for b in bs:
          ...
          args = list(map(lambda x: self.get(x, lambda: int(x)), args))
          # print(b.name, args, deps)
          
          adds = self.add_piece(name=b.name, args=args, deps=deps)
          # print('self.type2nodes', self.type2nodes[Line])
